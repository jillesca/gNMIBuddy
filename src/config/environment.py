...
class GNMIBuddySettings(BaseSettings):
    ...
    def get_logging_config(self) -> Dict[str, Any]:
        """
        Get logging configuration by delegating to existing logging environment system.

        This method integrates with the existing EnvironmentConfigReader to maintain
        backward compatibility while providing centralized access.

        Returns:
            Dictionary with logging configuration from environment variables
        """
        # Use lazy import to avoid circular dependency
        try:
            from ..logging.config.environment import EnvironmentConfigReader

            return EnvironmentConfigReader.read_configuration()
        except ImportError:
            # Fallback if import fails
            return {}

    def get_all_environment_variables(self) -> Dict[str, Optional[str]]:
        """
        Get all environment variables as a dictionary for inspection/debugging.
        ...
        return {
            ...
            "GNMIBUDDY_LOG_LEVEL": self.gnmibuddy_log_level,
            "GNMIBUDDY_MODULE_LEVELS": self.gnmibuddy_module_levels,
            "GNMIBUDDY_LOG_FILE": self.gnmibuddy_log_file,
            "GNMIBUDDY_STRUCTURED_LOGGING": (
                str(self.gnmibuddy_structured_logging)
                if self.gnmibuddy_structured_logging is not None
                else None
            ),
            "GNMIBUDDY_EXTERNAL_SUPPRESSION_MODE": self.gnmibuddy_external_suppression_mode,
            "GNMIBUDDY_MCP_TOOL_DEBUG": (
                str(self.gnmibuddy_mcp_tool_debug)
                if self.gnmibuddy_mcp_tool_debug is not None
                else None
            ),
        }
    ...
    def get_supported_variables(cls) -> Dict[str, str]:
        ...
        try:
            ...
            logging_vars = EnvironmentConfigReader.get_supported_variables()
        except ImportError:
            # Fallback if import fails
            logging_vars = {
        ...
        additional_vars = {
            "NETWORK_INVENTORY": "Path to network device inventory JSON file",
            "GNMIBUDDY_MCP_TOOL_DEBUG": "Enable debug logging for MCP tools (true/false)",
        }

        # Combine both dictionaries
        return {**logging_vars, **additional_vars}
